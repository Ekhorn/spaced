...
export function getOverlayPosition(
  editor: SolidEditor,
  range: BaseRange,
  { shouldGenerateOverlay, xOffset, yOffset }: GetSelectionRectsOptions,
): OverlayPosition {
  const [start, end] = Range.edges(range);
  const domRange = solidEditorToDomRangeSafe(editor, range);
  if (!domRange) {
    return {
      caretPosition: null,
      selectionRects: [],
  ...
  for (const [node, path] of nodeIterator) {
    const domNode = SolidEditor.toDOMNode(editor, node);
